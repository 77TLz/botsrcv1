...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5592988558765:+55 92988558765\n' // WhatsApp ID + phone number
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oii @${num.split('@')[0]SEJA BEM-VINDO , VAI TER QUE MAMAR OS ADMS , *${mdata.subject}*`
				...
				teks = `Adeus, @${num.split('@')[0]}adeus mamou gostoso seu filho da puta🤬👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Tá processando fdp enquanto isso vai descansar i espera a notificação chegar🇧🇼⌛',
				success: '✔️ Sucesso ✔️',
				...
				only: {
					...
					admin: '[❗]KKK ALA MEMBRO COMUM QUERENDO USAR COMANDO DE ADEMIR🤡🇧🇼',
			...
			const ownerNumber = ["@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5592988558765@s.whatsapp.net'
                        /******ApiKey Input******/
               ...
               break
				case 'demote':
                  ...
                  case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Removido com sucesso, Vaza Daqui Filho da puta levou ban pk mereceu :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'criador':
                  client.sendMessage(from, {displayname: 77 POHA", vcaurd: vcaurd}, MessageType.contact, { quoted: mek})
               client.sendMessage(from, 'NÚMERO DO MEU CRIADOR 5592988558765^^',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
	    ...
	    case 'kapankah':
			...
			case 'closegc':
				...
				case 'toimg':
                	...
                	client.updatePresence(from, Presence.composing) 
                        if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedVideo) return reply('❌ responda o video um fdp🤬❌')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
               ...
               if (anu.error) return reply(anu.error)
				...
				if (isUser) return reply(mess.only.daftarB)
					...
					break
                                case 'nsfwneko':
				    try{
						if (!isNsfw) return reply('❌ *PORNO DESLIGADO* ❌')
                                                if (!isUser) return reply(mess.only.daftarB)
                ...
                sem = sender.replace(5592988558765'@s.whatsapp.net','')
					...
					break
                                case 'afk':
                                        ...
                                        const tag = {
                                                ...
                                                contextInfo: { mentionedJid: [nom] AN GANGGU YA*`,
                                                }
                                        }
